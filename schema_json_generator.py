...
def make_json(field_string, final_json_name):
    json_formated_array = []
    temp_dict = {}
    ...
    with open("./jsons/" + final_json_name, "w", encoding="UTF-8") as f:
        json.dump(json_formated_array, f, ensure_ascii=False, indent=4)
    print(f"Created ./jsons/{final_json_name}")
...
def main():
    ...
    i = 0
    data = read_json(f"jsons/schemas_{table_name}.json")
